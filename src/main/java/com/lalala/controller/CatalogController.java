...
import com.lalala.vo.CatalogVO;
...
public class CatalogController {
	...
	public String listComments(@RequestParam(value="username",required=true)String username,Model model) {
		User user=(User) userDetailsService.loadUserByUsername(username); //登录验证用户
	 ...
	 */
	@PostMapping
	@PreAuthorize("authentication.name.equals(#catalogVO.username)") //指定用户才能进入
	public ResponseEntity<Response> create(@RequestBody CatalogVO catalogVO){
		String username=catalogVO.getUsername();  //通过分类获该用户
		Catalog catalog=catalogVO.getCatalog();  //获取分类
     ...
     */
	@DeleteMapping("/{id}")
	@PreAuthorize("authentication.name.equals(#username)")  //指定用户才能执行
